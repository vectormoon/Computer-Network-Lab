...
class CachingServer(TCPServer):
    ...
    def touchItem(self, path: str):
        ...
        ct = CacheTable()
        if (ct.getHeaders(path) is not None):
            headers = ct.getHeaders(path)
            body = ct.getBody(path)

            # with open('testCache_output.txt', 'a') as writer:
            #     writer.write("touchItem\n")
            #     writer.write(f"path\n")
            #     writer.writelines(str(ct.getHeaders(path)))
            
            return headers, body
        elif (ct.expired(path) or ct.getHeaders(path) is None):
            response = self.requestMainServer(path)
            #
            # with open('testCache_output.txt', 'a') as writer:
            #     writer.write("touchItem    ")
            #     # writer.write(f"{path}   {self.targetFiles[path]}")
            ...
            if (response is None):
                ...
                #
                # with open('testCache_output.txt', 'a') as writer:
                #     writer.write("touchItemRespond    ")
                #     writer.write(f"headers: {headers}\n   read: {body}")
                #     writer.write("\n")
                #
                return headers, body
...
class CachingServerHttpHandler(BaseHTTPRequestHandler):
    ...
    def sendHeaders(self):
        ...
        #     writer.write(str(headers))
        #     writer.writelines(str(body))
        #     writer.write("\n")
